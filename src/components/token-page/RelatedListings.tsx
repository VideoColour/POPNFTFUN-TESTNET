...
export default function RelatedListings({ excludedListingId }: { excludedListingId: bigint }) {
  ...
  useEffect(() => {
    fetchNFTs();
  }, [allNFTs, listingsInSelectedCollection, nftContract, excludedListingId]);
    ...
    if (listingsInSelectedCollection.length === 0) {
      ...
      mergedNfts = allNFTs.map((nft: any) => {
        ...
        const listing = listingsInSelectedCollection.find(
          (l: any) => l.tokenId.toString() === nftId && l.id !== excludedListingId
        );
        return listing
          ? {
              id: nftId,
  ...
  return (
    <Box mt="40px" textAlign="left" position="relative" className="custom-carousel">
      <Heading mb="30px">More from this collection</Heading>
      <Box
        borderTop="1px solid"
